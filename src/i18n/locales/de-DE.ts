export default {
  common: {
    confirm: 'Bestätigen',
    cancel: 'Abbrechen',
    save: 'Speichern',
    delete: 'Löschen',
    edit: 'Bearbeiten',
    search: 'Suchen',
    reset: 'Zurücksetzen',
    submit: 'Absenden',
    back: 'Zurück',
    loading: 'Laden...',
    success: 'Erfolg',
    error: 'Fehler',
    warning: 'Warnung',
    info: 'Info',
    refresh: 'Aktualisieren',
    deleteSuccess: 'Erfolgreich gelöscht',
    deleteFailed: 'Löschen fehlgeschlagen',
    downloadStarted: 'Download gestartet, prüfen Sie die Downloads',
    downloadFailed: 'Download fehlgeschlagen, Öffnen in neuem Fenster...',
    downloadRetry: 'Download fehlgeschlagen, bitte erneut versuchen',
    preparingDownload: 'Download vorbereiten...',
    loadFailed: 'Daten konnten nicht geladen werden',
    unknown: 'Unbekannt',
    confirmDelete: 'Löschen bestätigen',
    confirmDeleteMessage:
      'Möchten Sie den Analysebericht von "Technischer Bericht" wirklich löschen?',
    noPdfFile: 'Keine PDF-Datei für diesen Bericht verfügbar',
    noWordFile: 'Keine Word-Datei für diesen Bericht verfügbar',
    loginExpired: 'Anmeldung abgelaufen',
    noPermission: 'Keine Berechtigung für diese Ressource',
    notFound: 'Die angeforderte Ressource existiert nicht',
    serverError: 'Interner Serverfehler',
    requestFailed: 'Anfrage fehlgeschlagen',
    networkError: 'Netzwerkfehler, prüfen Sie Ihre Verbindung',
    requestConfigError: 'Anfragekonfigurationsfehler',
    pleaseUploadFile: 'Bitte laden Sie zuerst die Patentdatei hoch',
    pleaseCompleteInfo: 'Bitte füllen Sie alle Informationen aus',
    analysisSubmitted: 'Analyseaufgabe eingereicht, prüfen Sie Ergebnisse im Verlauf',
    analysisFailed: 'Analyse fehlgeschlagen',
    fileFormatError: 'Nur PDF-, DOC-, DOCX-Formate werden unterstützt!',
    fileSizeError: 'Dateigröße darf 10MB nicht überschreiten!',
    uploading: 'Datei wird hochgeladen...',
    uploadSuccess: 'Datei erfolgreich hochgeladen',
    uploadFailed: 'Datei-Upload fehlgeschlagen',
    notLoggedIn: 'Nicht angemeldet, bitte zuerst anmelden',
    reportGenerateSuccess: 'Vorgang erfolgreich',
    reportGenerateFailed: 'Berichterstellung fehlgeschlagen',
    pleaseEnterTechField: 'Bitte geben Sie die Beschreibung des technischen Bereichs ein',
    draftSaveSuccess: 'Entwurf erfolgreich gespeichert',
    draftSaveFailed: 'Entwurf speichern fehlgeschlagen',
    reportDownloading: 'Bericht wird heruntergeladen...',
    pleaseEnterAllRequired: 'Bitte füllen Sie alle erforderlichen Felder korrekt aus',
    generatingDraft: 'System analysiert Ihre technische Lösung und generiert Patententwurf...',
    draftSubmitted:
      'Patententwurf-Aufgabe eingereicht, überprüfen Sie Ergebnisse in der Verwaltung',
    generateFailed: 'KI-Generierung fehlgeschlagen, bitte erneut versuchen',
    formReset: 'Formular wurde zurückgesetzt',
    patent: 'Patent',
  },
  menu: {
    home: 'Startseite',
    dashboard: 'Dashboard',
    patentSearch: 'Patentsuche',
    quickSearch: 'Schnellsuche',
    advancedSearch: 'Erweiterte Suche',
    searchResults: 'Suchergebnisse',
    techReport: 'Technischer Bericht',
    newReport: 'Neuer Bericht',
    reportHistory: 'Verlauf',
    patentDraft: 'Patententwurf',
    newDraft: 'Neuer Entwurf',
    draftManage: 'Verwaltung',
    threeAnalysis: 'Dreifachanalyse',
    newAnalysis: 'Neue Analyse',
    analysisHistory: 'Verlauf',
    defenseSupport: 'Verteidigungsunterstützung',
    cases: 'Fälle',
    simulation: 'Simulation',
    strategy: 'Strategie',
    profile: 'Profil',
    about: 'Über',
  },
  defenseAnnotation: {
    title: 'Annotationsantwort',
    subtitle: 'Laden Sie Patentdokumente hoch, um Annotationsantwortmeinungen zu generieren',
    uploadTitle: 'Patentdokument hochladen',
    uploadText: 'Ziehen Sie Patentanmeldedokumente hierher,',
    clickUpload: 'klicken Sie zum Hochladen',
    fileFormatTip: 'Unterstützt PDF-, DOC-, DOCX-Formate, Dateigröße nicht größer als 10 MB',
    generateAnnotationReply: 'Annotationsantwort generieren',
    taskRecords: 'Aufzeichnungen',
    statusFilter: 'Statusfilter',
    annotationReply: 'Annotationsantwort',
    noTaskRecords: 'Keine Aufzeichnungen',
    startFirstTask: 'Laden Sie Patentdokumente hoch, um die erste Aufgabe zu starten',
    supportedFormatsError: 'Nur PDF-, DOC-, DOCX-Dateiformate werden unterstützt!',
    fileSizeLimitError: 'Dateigröße darf 10 MB nicht überschreiten!',
    uploadingInfo: 'Datei wird hochgeladen...',
    uploadSuccess: 'Datei erfolgreich hochgeladen',
    uploadFailed: 'Datei-Upload fehlgeschlagen',
    pleaseUploadFile: 'Bitte laden Sie zuerst Patentdokumente hoch',
    taskSubmitted: '{taskName} Aufgabe übermittelt, bitte überprüfen Sie die Ergebnisse in der Liste',
    taskSubmissionFailed: 'Aufgabenübermittlung fehlgeschlagen',
    noPDF: 'Diese Aufgabe hat keine PDF-Datei',
    noWord: 'Diese Aufgabe hat keine Word-Datei',
    defaultPrompt: 'Datei hochladen',
    annotationResult: 'Annotationsimulation',
  },
  auth: {
    login: 'Anmelden',
    register: 'Registrieren',
    logout: 'Abmelden',
    username: 'Benutzername',
    password: 'Passwort',
    confirmPassword: 'Passwort bestätigen',
    email: 'E-Mail',
    captcha: 'Captcha',
    getCaptcha: 'Captcha abrufen',
    rememberMe: 'Angemeldet bleiben',
    forgotPassword: 'Passwort vergessen',
    noAccount: 'Kein Konto?',
    hasAccount: 'Bereits ein Konto?',
    goToLogin: 'Anmelden',
    goToRegister: 'Registrieren',
    loginSuccess: 'Erfolgreich angemeldet',
    registerSuccess: 'Erfolgreich registriert',
    logoutSuccess: 'Erfolgreich abgemeldet',
    loginTitle: 'Patentplattform',
    loginSubtitle: 'Intelligente Patentlösung',
    registerTitle: 'Konto erstellen',
    registerSubtitle: 'Der Plattform beitreten',
    pleaseEnterUsername: 'Benutzername eingeben',
    pleaseEnterPassword: 'Passwort eingeben',
    pleaseEnterEmail: 'E-Mail eingeben',
    pleaseEnterCaptcha: 'Captcha eingeben',
    passwordNotMatch: 'Passwörter stimmen nicht überein',
    invalidEmail: 'Ungültiges E-Mail-Format',
    // Anmeldeseite
    userLogin: 'Benutzeranmeldung',
    loginPrompt: 'Melden Sie sich an',
    pleaseEnterUsernameOrPhone: 'Name/Telefon eingeben',
    loginLoading: 'Anmeldung...',
    loginButton: 'Anmelden',
    googleLogin: 'Mit Google anmelden',
    googleLoginInfo: 'Google-Anmeldefunktion wird entwickelt...',
    googleLoginSuccess: 'Google-Anmeldung erfolgreich',
    googleLoginFailed: 'Google-Anmeldung fehlgeschlagen',
    googleLoginError: 'Google-Anmeldedienst nicht geladen, bitte Seite aktualisieren',
    processingGoogleLogin: 'Google-Anmeldung wird verarbeitet...',
    or: 'oder',
    noAccountPrompt: 'Kein Konto?',
    professionalPlatform: 'Professionelle Plattform',
    oneStopService: 'Komplett-Patentservice',
    smartSearch: 'Intelligente Suche',
    smartSearchDesc: 'Intelligente Suche globaler Patentliteratur',
    threeAnalysisDesc: 'Analyse der Patentnovität, Kreativität und Nützlichkeit',
    patentDraftDesc: 'KI-gestützte Generierung von Patentanmeldungsunterlagen',
    techReportDesc: 'Technische Lösungspatentierbarkeitsanalysebericht',
    defenseSupportDesc: 'Patentverteidigungsstrategien und Fallunterstützung',
    // Registrierungsseite
    userRegister: 'Registrierung',
    registerPrompt: 'Erstellen Sie Ihr Konto',
    setPassword: 'Passwort festlegen',
    passwordStrength: 'Passwortstärke:',
    weak: 'Schwach',
    medium: 'Mittel',
    strong: 'Stark',
    registering: 'Registrierung...',
    registerButton: 'Registrieren',
    hasAccountPrompt: 'Bereits ein Konto?',
    // Formularvalidierungsnachrichten
    pleaseEnterCaptchaCode: 'Bitte Captcha eingeben',
    pleaseEnterUsernameOrEmail: 'Bitte Benutzername oder E-Mail eingeben',
    usernameMinLength: 'Benutzername muss mindestens 3 Zeichen lang sein',
    usernameLength: 'Benutzername sollte 3-20 Zeichen lang sein',
    usernamePattern: 'Benutzername darf nur Buchstaben, Zahlen und Unterstriche enthalten',
    passwordMinLength: 'Passwort muss mindestens 6 Zeichen lang sein',
    captchaRequired: 'Bitte Captcha eingeben',
    getCaptchaFailed: 'Fehler beim Abrufen des Captcha',
    pleaseGetCaptcha: 'Bitte zuerst Captcha abrufen',
    loginFailed: 'Anmeldung fehlgeschlagen, bitte Benutzername und Passwort überprüfen',
    registerSuccessMsg: 'Registrierung erfolgreich! Bitte melden Sie sich an',
    registerFailed: 'Registrierung fehlgeschlagen, bitte erneut versuchen',
  },
  patentSearch: {
    title: 'Schnelle Patentsuche',
    subtitle: 'Technische Lösung eingeben',
    searchConditions: 'Suchkriterien',
    patentTitle: 'Patenttitel',
    technicalSolution: 'Technische Lösung',
    pleaseEnterTitle: 'Titel eingeben',
    pleaseEnterSolution: 'Lösung eingeben',
    startSearch: 'Suche starten',
    searching: 'Suchen...',
    searchHistory: 'Suchverlauf',
    totalRecords: 'Gesamt {count} Einträge',
    noHistory: 'Kein Verlauf',
    publicationNumber: 'Veröffentlichungsnummer',
    publicationDate: 'Veröffentlichungsdatum',
    applicant: 'Antragsteller',
    downloadPDF: 'PDF herunterladen',
    downloadWord: 'Word herunterladen',
    viewDetail: 'Details anzeigen',
    completed: 'Abgeschlossen',
    generating: 'Generierung',
    failed: 'Fehlgeschlagen',
    imageLoadFailed: 'Ladefehler',
    clickToEnlarge: 'Klicken zum Vergrößern',
    tryExample: 'Beispiele ausprobieren:',
    searchFailed: 'Suche fehlgeschlagen',
    loadHistoryFailed: 'Fehler beim Laden des Suchverlaufs',
    patentSearchReport: 'Patentsuchbericht',
    // Zahlungsbezogen
    paid: 'Bezahlt',
    needPayment: 'Zahlung erforderlich',
    downloadReport: 'Bericht herunterladen - Zahlung erforderlich',
    reportName: 'Berichtsname:',
    fileFormat: 'Dateiformat:',
    price: 'Preis:',
    priceValue: '19,9€',
    paymentMethod: 'Zahlungsmethode',
    alipay: 'Alipay',
    wechatPay: 'WeChat Pay',
    generateQRCode: 'QR-Code generieren',
    confirmPayment: 'Zahlung bestätigen',
    paying: 'Wird verarbeitet...',
    scanQRCode: 'Bitte verwenden Sie {method}, um den QR-Code zu scannen',
    generatingQRCode: 'Zahlungs-QR-Code wird generiert...',
    orderNumber: 'Bestellnummer:',
    qrCodeLoadFailed: 'QR-Code konnte nicht geladen werden',
    paymentFailed: 'Zahlung fehlgeschlagen, bitte erneut versuchen',
    taskIdEmpty: 'Aufgaben-ID ist leer, Zahlung kann nicht verarbeitet werden',
  },
  techReport: {
    title: 'Technischer Bericht',
    newReport: 'Neuer Bericht',
    subtitle: 'Professionellen Bericht erstellen',
    technicalInfoInput: 'Informationseingabe',
    inventionName: 'Erfindungsname',
    technicalField: 'Technisches Gebiet',
    technicalSolution: 'Technische Lösung',
    pleaseEnterName: 'Name eingeben',
    pleaseEnterField: 'Gebiet eingeben',
    pleaseEnterSolution: 'Lösung beschreiben',
    generate: 'Generieren',
    generating: 'Generierung...',
    generationPreview: 'Vorschau',
    reportHistory: 'Berichtsverlauf',
    viewReport: 'Bericht anzeigen',
    downloadReport: 'Herunterladen',
    downloadPDF: 'PDF herunterladen',
    downloadWord: 'Word herunterladen',
    nameRequired: 'Name eingeben',
    fieldRequired: 'Gebiet eingeben',
    solutionRequired: 'Lösung ausfüllen',
    solutionMinLength: 'Mindestens 50 Zeichen',
    keyword: 'Stichwort',
    searchPlaceholder: 'Nach Titel suchen',
    status: 'Status',
    selectStatus: 'Status wählen',
    allStatus: 'Alle',
    statusGenerating: 'Generierung',
    statusCompleted: 'Abgeschlossen',
    statusFailed: 'Fehlgeschlagen',
    timeRange: 'Zeitraum',
    to: 'bis',
    startDate: 'Startdatum',
    endDate: 'Enddatum',
    noReports: 'Keine Berichte',
    createFirstReport: 'Ersten erstellen',
    imageLoadFailed: 'Ladefehler',
    clickToEnlarge: 'Vergrößern',
  },
  patentDraft: {
    title: 'Patententwurf',
    newDraft: 'Neuer Entwurf',
    subtitle: 'Automatische Generierung',
    inventionPatent: 'Erfindungspatent',
    professional: 'Professionell',
    fillInstructions: 'Anweisungen',
    fillDescription: 'Formular ausfüllen',
    draftList: 'Entwurfsliste',
    draftManagement: 'Verwaltung',
    patentDraftTitle: 'Patententwurf',
    manageDescription: 'Bearbeiten und herunterladen',
    searchPlaceholder: 'Suchen...',
    statusFilter: 'Statusfilter',
    allStatus: 'Alle',
    statusDraft: 'Entwurf',
    statusReviewing: 'In Überprüfung',
    statusCompleted: 'Abgeschlossen',
    totalDrafts: 'Gesamt {count} Entwürfe',
    createTime: 'Erstellt am',
    updateTime: 'Aktualisiert am',
    downloadPDF: 'PDF herunterladen',
    downloadWord: 'Word herunterladen',
    noPDFAvailable: 'Kein PDF verfügbar',
    noWordAvailable: 'Kein Word-Dokument verfügbar',
    noDrafts: 'Keine Entwürfe',
    createFirstDraft: 'Ersten erstellen',
    imageLoadFailed: 'Ladefehler',
    clickToEnlarge: 'Vergrößern',
    editDraft: 'Bearbeiten',
    inventionName: 'Name',
    technicalField: 'Gebiet',
    technicalSolution: 'Lösung',
    claims: 'Ansprüche',
    description: 'Beschreibung',
    abstract: 'Zusammenfassung',
    saveDraft: 'Speichern',
    submitReview: 'Absenden',
    pleaseEnterName: 'Name eingeben',
    nameLength: 'Erfindungsname sollte 3-50 Zeichen lang sein',
    pleaseEnterField: 'Gebiet eingeben',
    pleaseEnterSolution: 'Lösung beschreiben',
    clearAndRefill: 'Löschen',
    generateDraft: 'Generieren',
    generating: 'Generierung...',
    confirmDelete: '"{title}" löschen?',
    confirmDeleteTitle: 'Löschen bestätigen',
  },
  threeAnalysis: {
    title: 'Dreifachanalyse',
    newAnalysis: 'Neue Analyse',
    analysisHistory: 'Verlauf',
    novelty: 'Neuheit',
    creativity: 'Kreativität',
    practicality: 'Praktikabilität',
    analysisReport: 'Bericht',
    startAnalysis: 'Starten',
    analyzing: 'Analyse...',
    viewResult: 'Ergebnis',
  },
  notification: {
    title: 'Benachrichtigungen',
    markAllRead: 'Alle als gelesen markieren',
    markAsRead: 'Als gelesen markieren',
    clearAll: 'Alle löschen',
    noNotification: 'Keine Benachrichtigungen',
    systemNotification: 'System',
    newMessage: 'Neue Nachricht',
  },
  settings: {
    language: 'Sprache',
    theme: 'Design',
    changePassword: 'Passwort ändern',
    accountSettings: 'Einstellungen',
  },
  dashboard: {
    welcomeBack: 'Willkommen zurück, {user}!',
    user: 'Benutzer',
    todayIs: 'Heute ist {date}, lassen Sie uns mit der Arbeit von heute beginnen',
    newTechReport: 'Neuer Technischer Bericht',
    systemUsageGuide: 'Systemverwendungsanleitung',
    guideDescription:
      'Das intelligente Patentservice-System bietet Ihnen eine Komplettlösung für Patentanmeldungen. Bitte folgen Sie diesen Schritten, um das System zu verwenden:',
    tips: 'Tipps',
    guideTips:
      'Sie können das System von jedem Schritt aus beginnen. Das System wird entsprechende Serviceunterstützung basierend auf Ihren Anforderungen bieten.',
    guideSteps: {
      techReport: {
        title: 'Technischer Bericht',
        description:
          'Brauchen Sie eine Richtung? Lassen Sie uns Ihnen helfen, eine technische Lösung mit klaren Anweisungen und professionellem Ansatz zu erstellen.',
      },
      patentDraft: {
        title: 'Patententwurf',
        description:
          'Sparen Sie Zeit und Mühe. Generieren Sie Patententwürfe mit einem Klick und vervollständigen Sie Ihre Patentanmeldung einfach.',
      },
    },
    quickTools: {
      title: 'Schnellwerkzeuge',
      techReport: {
        title: 'Technischer Bericht',
        description: 'Schnell technische Lösungsanalyseberichte generieren',
      },
      patentDraft: {
        title: 'Patentschrift',
        description: 'Patentanmeldedokumente schreiben',
      },
    },
    recentActivities: 'Letzte Aktivitäten',
    viewAll: 'Alle anzeigen',
    view: 'Anzeigen',
    noActivityRecords: 'Keine Aktivitätsaufzeichnungen',
    feedback: {
      title: 'Feedback',
      selectType: 'Bitte Feedback-Typ auswählen',
      typeSuggestion: 'Funktionsvorschlag',
      typeIssue: 'Problemmeldung',
      typeOther: 'Sonstiges',
      placeholder: 'Bitte geben Sie Ihr Feedback oder Ihren Vorschlag ein (max. 1000 Zeichen)',
      contact: 'Kontakt (Optional)',
      submit: 'Feedback senden',
      exceedLimit: 'Feedback darf 1000 Zeichen nicht überschreiten',
      success: 'Vielen Dank für Ihr Feedback!',
    },
    todoList: 'Aufgabenliste',
    add: 'Hinzufügen',
    due: 'Fällig',
    noTodoItems: 'Keine Aufgaben',
    addTodoItem: 'Aufgabe hinzufügen',
    title: 'Titel',
    deadline: 'Frist',
    enterTodoTitle: 'Bitte geben Sie den Aufgabentitel ein',
    selectDeadline: 'Frist auswählen',
    cancel: 'Abbrechen',
    confirm: 'Bestätigen',
  },
  landing: {
    brand: 'PatentPro',
    nav: { features: 'Funktionen', pricing: 'Preise', about: 'Über uns', contact: 'Kontakt' },
    hero: {
      title: 'Intelligente Patentdienste leicht gemacht',
      subtitle:
        'Professionelle KI-gestützte Patentplattform für Patentsuche, Analyse, Erstellung und Verteidigung',
      getStarted: 'Jetzt starten',
      viewDemo: 'Demo ansehen',
      stats: {
        users: 'Aktive Nutzer',
        patents: 'Verarbeitete Patente',
        satisfaction: 'Zufriedenheit',
      },
      floatingCards: {
        techReport: 'Technischer Bericht',
        patentSearch: 'Patentsuche',
        patentDraft: 'Patententwurf',
      },
    },
    services: {
      title: 'Professionelle Lösungen für geistiges Eigentum',
      subtitle: 'Umfassende Patentdienste für Unternehmen und Einzelpersonen',
      intelligentAnalysis: {
        title: 'Intelligente Analyseplattform',
        description: 'KI-basierte Patentanalyse und Bewertungssystem',
      },
      globalSearch: {
        title: 'Globale Patentsuche',
        description: 'Suchdienst mit multinationalen Patentdatenbanken',
      },
      patentDraft: {
        title: 'Patentverfassungstool',
        description: 'KI-unterstütztes professionelles Dokumentengenerierungssystem',
      },
      threeAnalysis: {
        title: 'Dreifachanalyse-Service',
        description: 'Professionelle Bewertung von Neuheit, Kreativität und Praktikabilität',
      },
      techReport: {
        title: 'Erstellung technischer Berichte',
        description: 'Intelligente technische Lösungsanalyseberichte',
      },
      defenseSupport: {
        title: 'Verteidigungsunterstützung',
        description: 'Patentanmeldungsverteidigungsstrategie und Dokumentenunterstützung',
      },
    },
    stats: {
      title: 'Datengesteuerte Patentdienste',
      subtitle: 'Präzise Patentinformationen durch fortschrittliche Datenanalyse',
      patentDatabase: 'Patentdatenbank',
      successCases: 'Erfolgsfälle',
      customerSatisfaction: 'Kundenzufriedenheit',
    },
    features: {
      title: 'Leistungsstarke Funktionen für alle Anforderungen',
      subtitle:
        'Von der Suche bis zur Erstellung, von der Analyse bis zur Verteidigung - umfassende Lösungen',
      items: {
        techReport: {
          title: 'Technischer Lösungsbericht',
          desc: 'Intelligente Analyse und professionelle Patentfähigkeitsberichte',
        },
        patentSearch: {
          title: 'Intelligente Patentsuche',
          desc: 'Umfangreiche Patentdatenbank mit intelligenten Algorithmen',
        },
        patentDraft: {
          title: 'Intelligente Patentverfassung',
          desc: 'KI-unterstützte Erstellung von Patentanmeldungen',
        },
        threeAnalysis: {
          title: 'Dreifachanalyse',
          desc: 'Professionelle Bewertung von Neuheit, Kreativität und Praktikabilität',
        },
        defenseSupport: {
          title: 'Verteidigungsunterstützung',
          desc: 'Patentverteidigungsstrategien und Fallunterstützung',
        },
        aiPowered: {
          title: 'KI-Assistent',
          desc: 'Fortschrittliche KI-Technologie für intelligente Services',
        },
      },
    },
    pricing: {
      title: 'Wählen Sie den richtigen Plan',
      subtitle: 'Flexible Preispläne für individuelle und unternehmerische Bedürfnisse',
      popular: 'Beliebt',
      perMonth: '/Monat',
      choosePlan: 'Plan wählen',
      plans: {
        free: {
          name: 'Kostenlos',
          features: {
            searches: '5 Suchen/Monat',
            reports: '2 Berichte/Monat',
            support: 'Community-Support',
          },
        },
        pro: {
          name: 'Professional',
          features: {
            searches: 'Unbegrenzte Suchen',
            reports: '20 Berichte/Monat',
            drafts: '10 Entwürfe/Monat',
            support: 'Prioritäts-Support',
            api: 'API-Zugang',
          },
        },
        enterprise: {
          name: 'Enterprise',
          features: {
            unlimited: 'Unbegrenzte Nutzung',
            team: 'Team-Zusammenarbeit',
            custom: 'Anpassbare Dienste',
            priority: 'Dedizierter Support',
            dedicated: 'Dedizierter Server',
          },
        },
      },
    },
    cta: {
      title: 'Bereit loszulegen?',
      subtitle: 'Jetzt registrieren und intelligente Patentdienste erleben',
      startNow: 'Jetzt starten',
    },
    about: {
      title: 'Über uns',
      subtitle: 'Professionelle geistige Eigentumsdienstleistungen für globale Innovatoren',
      mission: {
        title: 'Unsere Mission',
        content:
          'Durch KI und Big-Data-Technologie Patentdienste intelligenter, effizienter und bequemer zu machen und Innovatoren zu helfen, ihren kreativen Wert zu schützen und zu realisieren.',
      },
      vision: {
        title: 'Unsere Vision',
        content:
          'Die weltweit führende intelligente Patentdienstleistungsplattform zu werden und starke Unterstützung für den Schutz des geistigen Eigentums für Innovation zu bieten.',
      },
      values: {
        title: 'Kernwerte',
        innovation: 'Innovationsgetrieben',
        innovationDesc:
          'Kontinuierliche technologische Innovation, Bereitstellung modernster Lösungen',
        professional: 'Professioneller Service',
        professionalDesc: 'Erfahrenes Expertenteam, professionell und zuverlässig',
        efficiency: 'Effizient & Bequem',
        efficiencyDesc: 'KI-unterstützt, Verbesserung der Serviceeffizienz',
        integrity: 'Integrität zuerst',
        integrityDesc: 'Berufsethik wahren, Kundeninteressen schützen',
      },
    },
    contact: {
      title: 'Kontaktieren Sie uns',
      subtitle: 'Wir sind immer bereit, Ihnen zu helfen',
      info: {
        title: 'Kontaktinformationen',
        address: 'Adresse',
        email: 'E-Mail',
        phone: 'Telefon',
        phoneContent: '+86 152-7318-4384',
        hours: 'Geschäftszeiten',
        hoursContent: 'Montag bis Freitag 9:00-18:00',
      },
      form: {
        title: 'Online-Anfrage',
        name: 'Name',
        namePlaceholder: 'Bitte geben Sie Ihren Namen ein',
        email: 'E-Mail',
        emailPlaceholder: 'Bitte geben Sie Ihre E-Mail ein',
        subject: 'Betreff',
        subjectPlaceholder: 'Bitte geben Sie den Betreff ein',
        message: 'Nachricht',
        messagePlaceholder: 'Bitte geben Sie Ihre Nachricht ein',
        submit: 'Nachricht senden',
      },
      social: {
        title: 'Folgen Sie uns',
      },
    },
    footer: {
      description:
        'Professionelle Plattform für umfassende Lösungen zum Schutz des geistigen Eigentums',
      product: 'Produkt',
      company: 'Unternehmen',
      legal: 'Rechtliches',
      api: 'API-Dokumentation',
      blog: 'Blog',
      privacy: 'Datenschutz',
      terms: 'Nutzungsbedingungen',
      copyright: 'Alle Rechte vorbehalten',
    },
  },
}
